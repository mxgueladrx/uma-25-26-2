...
public class ServicioCitas {
    ...
    @Value("${citas.duracion.maxima:120}") // estaba en 15 y era igual que el minimo
    ...
    public Cita crear(Cita cita) {
        if (cita.getDuracion() == null || cita.getDuracion() < duracionMinima || cita.getDuracion() > duracionMaxima) {
    ...
    public Cita confirmar(Long id) {
        ...
        return repositorio.save(cita); // hay que guardar los cambios en la bd
    }
    ...
    private boolean hayConflicto(LocalDateTime inicio, LocalDateTime fin) {
        return repositorio.findAll().stream().anyMatch(c ->
            c.getEstado() != EstadoCita.CANCELADA &&
                inicio.isBefore(c.getFin()) && // es un and en vez de or sino todas las citas dan conflitos
